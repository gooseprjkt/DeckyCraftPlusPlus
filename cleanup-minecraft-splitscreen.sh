...
# =============================================================================
# Source vars.sh for launcher configuration
...
readonly LAUNCHER_FLATPAK_ID_VAR="${LAUNCHER_FLATPAK_ID:-org.prismlauncher.PrismLauncher}"
...
cleanup_flatpaks() {
    ...
    # Check and remove launcher Flatpak (PineconeMC or PrismLauncher)
    if flatpak list --app 2>/dev/null | grep -q "$LAUNCHER_FLATPAK_ID_VAR"; then
        if [[ "$DRY_RUN" == true ]]; then
            print_dry "Uninstall Flatpak: ${LAUNCHER_NAME:-Launcher} ($LAUNCHER_FLATPAK_ID_VAR)"
        else
            print_info "Uninstalling ${LAUNCHER_NAME:-Launcher} Flatpak..."
            if flatpak uninstall -y --noninteractive "$LAUNCHER_FLATPAK_ID_VAR" 2>/dev/null; then
                print_success "Removed ${LAUNCHER_NAME:-Launcher} Flatpak"
            else
                print_warning "Failed to remove ${LAUNCHER_NAME:-Launcher} Flatpak (may need: flatpak uninstall $LAUNCHER_FLATPAK_ID_VAR)"
            fi
        fi
    else
        print_info "${LAUNCHER_NAME:-Launcher} Flatpak not installed"
    fi

    # Also check for PrismLauncher Flatpak (for users switching from PrismLauncher)
    if [[ "$LAUNCHER_FLATPAK_ID_VAR" != "$PRISM_FLATPAK_ID" ]]; then
...
show_summary() {
    ...
    echo "  - ${launcher_display_name} data and AppImage (~/.local/share/${launcher_display_name})"
    echo "  - ${launcher_display_name} Flatpak data (~/.var/app/${LAUNCHER_FLATPAK_ID_VAR})"
    ...
    echo "  - ${launcher_display_name} Flatpak application"
    echo ""
...
show_detected_components() {
    ...
    if command -v flatpak &>/dev/null; then
        if flatpak list --app 2>/dev/null | grep -q "$LAUNCHER_FLATPAK_ID_VAR"; then
            print_info "${launcher_display_name} Flatpak app installed"
            found_any=true
        fi
        if flatpak list --app 2>/dev/null | grep -q "$PRISM_FLATPAK_ID"; then
