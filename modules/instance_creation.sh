...
create_instances() {
    ...
    for i in {1..4}; do
        ...
        # Check if launcher executable exists and is accessible
        local launcher_exec
        if launcher_exec=$(get_executable) && [[ -x "$launcher_exec" ]]; then
            # Try multiple CLI creation approaches with progressively fewer parameters
            # This handles different launcher versions that may have varying CLI support
            ...
            if "$launcher_exec" --cli create-instance \
                --name "$instance_name" \
            ...
            elif "$launcher_exec" --cli create-instance \
                --name "$instance_name" \
                --mc-version "$MC_VERSION" \
            ...
            print_info "Launcher executable not available, using manual method"
        fi
