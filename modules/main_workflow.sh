...
#   - launcher_setup.sh (for download_launcher, verify_cli)
...
#
# @changelog
#   3.0.4 (2026-03-31) - Added PineconeMC support via vars.sh configuration
...
main() {
    ...
    # Only download launcher if we don't have a creation launcher yet
    if [[ -z "$CREATION_LAUNCHER" ]]; then
        download_launcher        # Download launcher AppImage for CLI automation
    fi

    if [[ -n "$CREATION_EXECUTABLE" ]] && ! verify_cli; then
        print_info "${LAUNCHER_NAME:-Launcher} CLI unavailable - will use manual instance creation"
    fi
